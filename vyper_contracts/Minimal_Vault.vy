...
def initialize(
    ...
    @dev
        If `nameOverride` is not specified, the name will be 'concave'
        combined with the name of `token`.

        If `symbolOverride` is not specified, the symbol will be 'cv'
    ...
    if symbolOverride == "":
        self.symbol = concat("cv", DetailedERC20(token).symbol())
    else:
    ...
    self.DOMAIN_SEPARATOR = keccak256(
        concat(
            DOMAIN_TYPE_HASH,
            keccak256(convert("Concave Vault", Bytes[11])),
...
def apiVersion() -> String[28]:
    ...
    @notice
        ...
        can use this version number to compare with Concave's GitHub and
...
def report(gain: uint256, loss: uint256, _debtPayment: uint256) -> uint256:
    ...
    @return Amount of debt outstanding (if totalDebt > debtLimit).
    """
    ...
    if self.strategies[msg.sender].debtRatio == 0:
